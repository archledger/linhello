...
/// Idempotent ORT init. Mirrors `aegyra-biometrics::ort_init` so we don't
...
fn ensure_ort() -> Result<()> {
    ...
    ORT_INIT.call_once(|| {
        ...
        if let Err(e) = ort::init().with_name("aegyra-liveness").commit() {
            err = Some(format!("ort init: {e}"));
        }
    });
    if let Some(e) = err {
        return Err(AegyraError::Biometrics(e));
    }
    Ok(())
}
