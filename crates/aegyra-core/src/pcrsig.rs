...
/// Locate the signature JSON, honouring `AEGYRA_PCR_SIGNATURE` for test/dev.
pub fn discover_signature_path() -> Option<PathBuf> {
    if let Some(p) = std::env::var_os("AEGYRA_PCR_SIGNATURE") {
...
/// Locate the public-key PEM, honouring `AEGYRA_PCR_PUBKEY` for test/dev.
pub fn discover_pubkey_path() -> Option<PathBuf> {
    if let Some(p) = std::env::var_os("AEGYRA_PCR_PUBKEY") {
        let p = PathBuf::from(p);
        return p.exists().then_some(p);
    }
...
pub fn load_pubkey_pem() -> Result<String> {
    let path = discover_pubkey_path()
        .ok_or_else(|| AegyraError::Policy("no TPM2 PCR public key found".into()))?;
    std::fs::read_to_string(&path).map_err(AegyraError::Io)
}
...
pub fn load_signatures(bank: &str) -> Result<Vec<PcrSignature>> {
    let path = discover_signature_path()
        .ok_or_else(|| AegyraError::Policy("no TPM2 PCR signature file found".into()))?;
    let text = std::fs::read_to_string(&path).map_err(AegyraError::Io)?;
...
pub fn parse_signatures(text: &str, bank: &str) -> Result<Vec<PcrSignature>> {
    let raw: HashMap<String, Vec<RawEntry>> =
        serde_json::from_str(text).map_err(|e| AegyraError::Serde(e.to_string()))?;
    ...
    entries
        ...
        .map(|e| {
            Ok(PcrSignature {
                ...
                sig: STANDARD
                    .decode(&e.sig)
                    .map_err(|err| AegyraError::Serde(format!("bad signature base64: {err}")))?,
            })
        })
        .collect()
}
...
fn from_hex(s: &str) -> Result<Vec<u8>> {
    if s.len() % 2 != 0 {
        return Err(AegyraError::Serde("odd-length hex string".into()));
    }
    (0..s.len())
        .step_by(2)
        .map(|i| {
            u8::from_str_radix(&s[i..i + 2], 16)
                .map_err(|e| AegyraError::Serde(format!("bad hex: {e}")))
        })
        .collect()
}
