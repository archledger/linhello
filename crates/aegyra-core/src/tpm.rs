...
fn rsa_pem_to_public(pubkey_pem: &str) -> Result<Public> {
    ...
    let key = rsa::RsaPublicKey::from_public_key_pem(pubkey_pem)
        .map_err(|e| AegyraError::Policy(format!("parse PCR public key: {e}")))?;
    ...
    let key_bits = match modulus.len() * 8 {
        ...
        other => {
            return Err(AegyraError::Policy(format!(
    ...
    let exponent = {
        ...
        let mut buf = [0u8; 4];
        if e.len() > 4 {
            return Err(AegyraError::Policy("PCR key exponent too large".into()));
        }
