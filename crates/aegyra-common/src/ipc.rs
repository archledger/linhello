...
pub enum Request {
    ...
    /// Probe the host for the hardware/software Aegyra needs (TPM, Secure
...
pub enum CapabilityStatus {
    ...
    Warn,
    /// Absent — blocks Aegyra if the check is `required`.
    Missing,
}
...
pub struct CapabilityCheck {
    ...
    pub detail: String,
    /// If true, `Missing` means Aegyra cannot function.
    pub required: bool,
}
...
impl CapabilityReport {
    /// Can Aegyra run at all? True unless a required capability is missing.
