...
//
// Aegyra runs on internal laptop sensors *and* external UVC cameras, including
...
// by precedence: explicit env var → /etc/aegyra/cameras.conf → auto-detect →
...
/// dropped, so `aegyra doctor` can show them.
pub fn enumerate() -> Vec<CameraInfo> {
    v4l::context::enum_devices()
        ...
        .filter_map(|node| {
            ...
            let trusted = aegyra_liveness::device::validate_camera_device(&path).score > 0.0;
            Some(CameraInfo {
                name: node.name(),
                path,
                kind,
...
fn resolve_rgb() -> String {
    if let Some(p) = env_device("AEGYRA_RGB_DEVICE") {
        return p;
    }
...
fn resolve_ir() -> Option<String> {
    if let Some(p) = env_device("AEGYRA_IR_DEVICE") {
        return exists(p);
    }
...
/// Read `rgb=`/`ir=` from `/etc/aegyra/cameras.conf` (simple `key=value`,
/// `#` comments). Lets a user pin an external camera across reboots.
fn conf_device(key: &str) -> Option<String> {
    let path = std::path::Path::new(aegyra_common::CONFIG_ROOT).join("cameras.conf");
