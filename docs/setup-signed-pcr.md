...
By default Aegyra binds its TPM secrets to **PCR 7 only** (Secure Boot state),
...
across kernel updates. Aegyra consumes the **same** artifacts systemd produces:
`/run/systemd/tpm2-pcr-signature.json` + `/run/systemd/tpm2-pcr-public-key.pem`.

> Prerequisite: Secure Boot ON and a **UKI** boot (Aegyra `aegyra doctor`
...
> arbitrary PCR state. Aegyra mitigates this by *also* binding **PCR 7** in the
...
Both must exist. `aegyra doctor` should now report **Signed PCR policy: [ OK ]**.

## 5. Re-seal Aegyra onto the Authorized tier

Aegyra picks the tier at seal time. Once the artifacts exist, re-seal so the
secrets move from PCR-7-literal to signed PCR-11:

```
sudo aegyra reseal-user-envelopes --user "$USER"   # password + template key
# If a new key/template is needed (e.g. coming from drift), re-enroll instead:
#   sudo aegyra enroll --user "$USER" --reset
#   sudo aegyra seal-password "$USER"
aegyra status      # security level should now read "Full"
aegyra diag        # tracked PCRs should include 11; no drift
```
...
sudo reboot
aegyra verify --user "$USER"   # must still match — NO re-seal, NO re-enroll
```
...
## How Aegyra uses these files

- `aegyra-core::pcrsig` discovers `tpm2-pcr-signature.json` /
  `tpm2-pcr-public-key.pem` (search order `/etc/systemd`, `/run/systemd`,
  `/usr/lib/systemd`; override with `AEGYRA_PCR_SIGNATURE` / `AEGYRA_PCR_PUBKEY`).
- `aegyra-core::policy::plan()` selects `Authorized([7,11])` when Secure Boot is
...
`journalctl -u aegyrad` around the failure — the message will indicate whether
